...
- **Configuration module** — Runtime-only gateway base URL resolution via `ACTSIS_LITELLM_URL`, global config (`~/.pi/agent/actsis-litellm.json`), project-local config, or `/login` prompt. Configurable catalog TTL and request timeout.
