...
- **Extra commands** — `/actsis-litellm:status` (credential/cache/provider state), `/actsis-litellm:models` (force catalog resync and report added/removed), `/actsis-litellm:logout` (revoke refresh token and clear local credentials).
